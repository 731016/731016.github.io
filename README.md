...
2025.04.20 - 2026.04.20
南方运营中心 Java开发工程师 武汉

负责军工行业离散制造装配行业MES、ERP系统和的开发与维护工作，重点负责工时管理、质量管理、采购管理、库存管理等核心模块的设计与实现。
...
ERP系统覆盖销售、计划、采购、生产、库存、财务六大模块，以MRP计划引擎驱动产销协同，实现从订单到交付、从采购到结算的全流程数字化管理。

**责任描述**

主要参与MRP计划、采购到检验、生产执行等关键业务流程的开发、实施和运维。

参与项目的需求分析和功能设计，确保系统功能满足需求。

与客户保持沟通，收集反馈并持续优化系统功能。

**技术描述**

+ MRP核心算法实现：设计基于BOM多级展开与提前期倒排的物料需求计算引擎，采用递归算法解析产品结构，结合Oracle存储过程处理复杂净需求运算，支撑物料高频计划运算。
+ 数据一致性保障：针对库存扣减、工单领料等并发操作，采用Redis分布式锁机制，防止重复扣料，保证数据准确性。
+ 针对ERP系统高频业务场景（如MRP运算、库存实时查询），基于Feign实现服务间调用熔断降级策略。配置Hystrix/Sentinel熔断规则，当下游服务（如库存中心、采购中心）响应超时或异常时，自动触发降级逻辑，返回兜底数据或缓存结果，避免级联故障，保障核心业务流程连续性。
+ 针对生产质量检验场景复杂表单需求，集成帆软FineReport报表工具。通过拖拽式设计复杂填报模板，支持检验数据批量录入、自动计算合格率、异常数据高亮预警，替代传统Excel手工填报，提升质量数据采集效率与准确性。
